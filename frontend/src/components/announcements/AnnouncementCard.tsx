...
import { createPdfHtmlPage, fetchPdfBlob } from "@/components/fees/document-viewer/utils/pdfUtils";
...
export function AnnouncementCard({ announcement, canDelete, onDelete, isDeleting }: AnnouncementCardProps) {
  ...
  const isImage = announcement.fileUrl ? isImageUrl(announcement.fileUrl) : false;
  ...
  const imageDownloadFileName =
    hasAttachment && announcement.fileUrl && isImage
      ? (() => {
          const extension = announcement.fileUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i)?.[0] || ".jpg";
          const sanitizedTitle = announcement.title.replace(/[^a-z0-9]/gi, "_").toLowerCase();
        ...
        <div className="flex flex-col sm:flex-row gap-0">
          ...
          <div className="sm:w-20 sm:min-w-20 flex sm:flex-col items-center justify-center gap-1 sm:gap-1 px-2 py-0.5 sm:py-1">
            <div className="text-center">
              ...
              <div className="text-2xl sm:text-3xl font-bold text-foreground leading-none my-1">{day}</div>
              <div className="text-xs sm:text-sm text-muted-foreground font-medium">{year}</div>
              <div className="text-xs text-muted-foreground/70 mt-1 hidden sm:block">{time}</div>
            </div>
          </div>
            ...
            <Link to={`/announcements/${announcement._id}`} className="block group">
              ...
              <div className="flex items-start justify-between gap-3 mb-0">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed line-clamp-2 mb-0">{announcement.message}</p>
                </div>
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <div className="text-[10px] font-medium text-muted-foreground bg-muted/10 px-1 py-0.5 rounded">
                    {announcement.postedBy.role
                      ? announcement.postedBy.role.charAt(0).toUpperCase() + announcement.postedBy.role.slice(1).toLowerCase()
                      : ""}
                  </div>
                  {canDelete && (
                    <Button
