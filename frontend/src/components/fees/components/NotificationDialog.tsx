...
interface NotificationDialogProps {
  ...
  onSend: (emailType: "hostelFee" | "messFee" | "both", notes?: string) => Promise<void>;
}
...
export function NotificationDialog({
  ...
  const [emailType, setEmailType] = useState<"hostelFee" | "messFee" | "both">("both");
  const [notes, setNotes] = useState("");
  const [isSending, setIsSending] = useState(false);

  const canSendHostel = hostelFeeStatus === "documentNotSubmitted" || hostelFeeStatus === "rejected";
  const canSendMess = messFeeStatus === "documentNotSubmitted" || messFeeStatus === "rejected";
  const canSendBoth = canSendHostel && canSendMess;

  // Determine available options
  const availableOptions: Array<{ value: "hostelFee" | "messFee" | "both"; label: string }> = [];
  if (canSendHostel) {
  ...
  if (canSendBoth) {
    availableOptions.push({ value: "both", label: "Both Hostel and Mess Fees" });
  }
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent>
        <SheetHeader>
          ...
          <SheetDescription>
            Send a reminder notification to {studentName} to submit their fee documents.
          </SheetDescription>
        </SheetHeader>
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Select
              value={emailType}
              onValueChange={(value) => setEmailType(value as "hostelFee" | "messFee" | "both")}
            >
